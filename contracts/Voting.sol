...
contract Election {
    ...
    function addCandidate(
        ...
        require(
            elections[electionId].state == ElectionState.NotStarted,
            "Cannot add candidates after election started"
        );
    ...
    function addVoter(uint256 electionId, address _voter) public onlyOwner {
        require(
            elections[electionId].state == ElectionState.NotStarted,
            "Cannot add voters after election started"
        );
    ...
    function declareWinner(
        ...
        if (election.winningCandidateIds.length > 1) {
            ...
            uint256 randomIndex = uint256(
                keccak256(abi.encodePacked(blockHash))
            ) % election.winningCandidateIds.length;
            election.winningCandidateIds = [
                election.winningCandidateIds[randomIndex]
            ]; // Choose one winner
        }
    ...
    function vote(uint256 electionId, uint256 _candidateId) public {
        ...
        require(
            election.voters[msg.sender].isAuthorized,
            "You are not an authorized voter"
        );
        require(
            !election.voters[msg.sender].hasVoted,
            "You have already voted"
        );
        require(
            _candidateId < election.candidatesCount,
    ...
    function getAllElections()
        public
        view
        returns (uint256[] memory, string[] memory, ElectionState[] memory)
    {
    ...
    function deleteElection(uint256 electionId) public onlyOwner {
        require(
            elections[electionId].state == ElectionState.Ended,
            "Only ended elections can be deleted"
        );
